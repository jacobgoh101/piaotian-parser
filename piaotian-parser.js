...
module.exports = function (ctx, req, res) {
  ...
  request({url: articleUrl, encoding: null, timeout: 60000, maxAttempts: 6000, retryDelay: 1000}).then(HTML => {
    let html = HTML;
    html = iconv.decode(html, "gb2312");
    const $ = cheerio.load(html);
    const isContentTable = () => {
      return $('title')
        .text()
        .indexOf('无弹窗全文阅读') > -1
    };
    ...
    $('a:contains("返回目录")').each((i, elem) => {
      $(elem).attr('target', '_blank');
      let href = $(elem).attr("href");
      let currentUrl = `https://${endpointUrl}/piaotian-parser?url=${articleUrl}`;
      href += `#${currentUrl}`;
      $(elem).attr("href", href);
    });
